...
static constexpr int _max_response_size = 1024*1024*280;
...
static void read_html_file_and_prepare_response( 
	char *file_name, char *html_source_dir, bool is_browser_request, ResponseWrapper &response );

static void request_spider( 
	callback_ptr _callback, char *post, ResponseWrapper &response, ServerDataWrapper &sdw );
  ...
  char *html_source_dir, callback_ptr callback )
{
	...
	bool get;
	bool options;
	int uri_status = get_uri_to_serve(
		socket_request_buf, socket_request_buf_size, uri, _uri_buf_size, &get, &post, &options
	);
	...
	if (header_sending_result == SOCKET_ERROR) 
	{ 	// If error...
...
static void request_spider( callback_ptr callback, char *post, ResponseWrapper &response, ServerDataWrapper &sdw )
{
	...
	if( callback_return < 0 || sdw.sd.sp_response_buf_size == 0 ) 
	{ 	// An error 
...
static void read_html_file_and_prepare_response( char *file_name, char *html_source_dir, bool is_browser_request, ResponseWrapper &response )
{
    ...
    if (fin) 
        ...
        if( file_size > 0 ) 
				{
            if( file_size <= _file_to_serve_buf_size ) 
						{ 	// The static buffer is big enough...
